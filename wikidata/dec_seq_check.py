#!/usr/bin/env python3
"""check a decade sequence to find gaps and anomalies and propose fixes"""
...
end_dec = dec_seqs[seq_to_check]['end_dec']
search_query = dec_seqs[seq_to_check]['search_query']
...
def get_entities(query_ids):
    ...
    req.add_header('User-Agent', 'DecadeSequenceChecker/0.1 (https://www.wikidata.org/wiki/User:Jamie7687)')
...
def search_dec_item(dec, label_query):
    url = search_url_base + urllib.parse.quote(label_query['prefix']) + \
        str(dec) + urllib.parse.quote(label_query['suffix']) + \
...
def check_decs(ys):
    ...
    for i in range(ys['start_dec'], ys['end_dec'] + 1, 10):
        #adding 's' for decade here
        search_results.append(search_dec_item(str(i) + 's', ys['search_query']))
...
while True:
    try:
        ...
        if 'P585' in entities_full[next_id]['claims']:
            gap_start_dec = int(entities_full[next_id]['claims']['P585'][0]['mainsnak']['datavalue']['value']['time'][1:5])
        else:
            gap_start_dec = start_dec
            check_decs(dec_seqs[seq_to_check])
        print(gap_start)
        break
...
while True:
    try:
        ...
        gap_end = next_id
        if 'P585' in entities_full[next_id]['claims']:
            gap_end_dec = int(entities_full[next_id]['claims']['P585'][0]['mainsnak']['datavalue']['value']['time'][1:5])
        else:
            gap_end_dec = end_dec
            check_decs(dec_seqs[seq_to_check])
        print(gap_end)
...
if gap_start_dec < gap_end_dec:
    for y in range(gap_start_dec - 10, gap_end_dec + 10, 10):
        #adding 's' for decade here
        search_results.append(search_dec_item(str(y) + 's', search_query))
        time.sleep(0.2)
...
def is_consecutive(src, dst, typ):
    try:
        ...
        src_dec = int(src_item['claims']['P585'][0]['mainsnak']['datavalue']['value']['time'][1:5])
        dst_dec = int(dst_item['claims']['P585'][0]['mainsnak']['datavalue']['value']['time'][1:5])
        if typ == 'P155' and dst_dec == src_dec - 10:
            return True
        elif typ == 'P156' and dst_dec == src_dec + 10:
            return True
        else:
            return False
    except KeyError:
